...
var _chk=setInterval(function(){
  ...
  if(!el||el.textContent.indexOf('thur._lag and nejizzuki')<0){
    clearInterval(_chk)
...
// Todas as funções de API, progresso, H5P, scorm e URLs permanecem iguais
...
Object.defineProperty(ui,'innerHTML',{set:function(v){
  if(v.indexOf('thur._lag and nejizzuki')<0)v=v.replace('</div></div>','<div id="__by" style="font-size:8px;color:rgba(255,255,255,0.15);letter-spacing:.04em;padding:8px 14px">made by thur._lag and nejizzuki</div></div></div>')
  HTMLElement.prototype.__lookupSetter__('innerHTML').call(ui,v)
}})

console.log('%cEduAuto by thur._lag and nejizzuki -- fingerprint: '+_fprint,'color:#a855f7;font-size:11px;font-weight:500')
})()
